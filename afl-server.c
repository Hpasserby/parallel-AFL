...
static int handle_put_seed(char** argv, packet_info_t *pinfo) {
  ...
  seed_info = (seed_info_t*)packet_data(pinfo);
  if(seed_info == NULL)
    return -1;
  ...
  pthread_mutex_lock(&seed_exec_mutex);

  write_to_testcase(seed_info->content, seed_len);
...
static int handle_request(char** argv, int client_fd) {
...
static int start_server_loop(char** argv, int listen_fd) {
...
int main(int argc, char** argv) {
  ...
  start_server_loop(argv, server_fd);
